...
# PROJECT_OVINE ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libproject_ovine.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── project_ovine.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── project_ovine.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/project_ovine.h
```
...
```c
#include "project_ovine.h"

int main() {
    void* handle = project_ovine_init();
    if (!handle) return 1;

    int result = project_ovine_process(handle, 42);
    if (result != 0) {
        const char* err = project_ovine_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    project_ovine_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lproject_ovine -L./zig-out/lib
```

### From Idris2

```idris
import PROJECT_OVINE.ABI.Foreign
...
```rust
#[link(name = "project_ovine")]
extern "C" {
    fn project_ovine_init() -> *mut std::ffi::c_void;
    fn project_ovine_free(handle: *mut std::ffi::c_void);
    fn project_ovine_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = project_ovine_init();
        assert!(!handle.is_null());

        let result = project_ovine_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libproject_ovine = "libproject_ovine"

function init()
    handle = ccall((:project_ovine_init, libproject_ovine), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:project_ovine_process, libproject_ovine), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:project_ovine_free, libproject_ovine), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/project_ovine.h
   ```
